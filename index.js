...
let person = {
  ...
  country: "Spain",
  birthDay: new Date(),
  hobbies: ["skiing", "mountain climbing", "singing"],
  addHobby(hobby) {
    return this.hobbies.push(hobby);
  },
  randomHobby() {
    let random = Math.floor(Math.random() * this.hobbies.length);
    return this.hobbies[random];
  },
  printName() {
    log(
...
log(person.randomHobby());
log(person.addHobby("skating"));
log(person.hobbies);
log(person.birthDay);
// person.printName();
